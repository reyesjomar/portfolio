# Jomar Reyes – Data Analytics Portfolio

Welcome to my data analytics portfolio! I'm a data analyst with a background in Kinesiology, bringing a human-centered approach to data storytelling. I use tools like SQL, Excel, Tableau, and Python to turn data into actionable insights that drive smarter decisions.

This portfolio includes projects developed through my training, bootcamps, and client simulations. Each project focuses on solving real-world problems using clean, well-documented code and visualizations.

---

## Featured Projects

### 1. Telecom Reporting Optimization  
**Tools:** SQL, Excel, Tableau  
Analyzed ad performance data to improve ROI and ad strategy for a telecom client.  
[View Project ➜](./project-telecom-reporting-optimization)

### 2. NYC Evictions Analysis  
**Tools:** Excel, Tableau  
Explored trends in NYC evictions and legal possession processes to recommend housing policy strategies.  
[View Project ➜](./project-nyc-evictions)

### 3. Maximalist vs. Traditional Running Shoes  
**Tools:** Excel, Vicon Motion Capture  
Compared kinematic performance to assess injury risks related to shoe design.  
[View Project ➜](./project-maximalist-vs-traditional-shoes)

---

## Tools & Skills

- SQL • Excel • Tableau • Python  
- Data Cleaning • KPI Analysis • Dashboard Design  
- Research & Reporting • Storytelling with Data

---

## 📬 Connect With Me

- [LinkedIn](linkedin.com/in/jomarreyes)
- [GitHub](github.com/reyesjomar)
